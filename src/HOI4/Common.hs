...
scope s = local $ \st ->
    let oldststate = gHOI4 st
    in  st { gHOI4 = oldststate { scopeStack = s : scopeStack oldststate } }

-- Get current scope, if there is one.
getCurrentHOI4Scope :: Monad m => PPT m (Maybe HOI4Scope)
getCurrentHOI4Scope = asks (listToMaybe . scopeStack . gHOI4)
